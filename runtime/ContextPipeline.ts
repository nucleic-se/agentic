...
function count(value: number): number {
    if (!Number.isFinite(value) || value < 0) {
        throw new RangeError('Token counter must return a non-negative finite estimate');
    }
...
export function estimateContextTokens(
    ...
    for (const source of input.messages) {
        ...
        messageTokens += count(counter.countTokensForMessages([{
            role: message.role,
            content: rich ? rich.filter(block => block.type === 'text').map(block => ({ ...block })) : message.content
        }]));
        if (rich) {
            messageTokens += rich.filter(block => block.type === 'image').length * imageCost;
        }
        ...
        if (message.role === 'tool_result') {
            messageTokens += count(counter.countTokens(JSON.stringify({
...
export async function collectPromptSections<T extends PromptContributionContext>(
    ...
    for (const contributor of contributors) {
        ...
        if (ids.has(contributor.id)) {
            throw new Error(`Duplicate prompt contributor: ${contributor.id}`);
        }
...
function groups(messages: readonly Message[]): Group[] {
    ...
    for (let index = 0; index < messages.length; index++) {
        unboundCount.push(unboundCount[index] + (unbound.has(index) ? 1 : 0));
    }
    const result: Group[] = [];
    for (let first = 0; first < messages.length;) {
        ...
        result.push({
            messages: structuredClone(messages.slice(first, end + 1)),
            firstIndex: first,
...
export async function composeAgentContext(input: ContextCompositionInput, options: ContextCompositionOptions = {}): Promise<ContextCompositionResult> {
    if (options.protectUserMessages !== undefined && !['latest', 'all'].includes(options.protectUserMessages)) {
        throw new TypeError('Invalid human-message retention policy');
    }
    ...
    if (options.protectUserMessages === 'latest') {
        input.messages.forEach((message, index) => {
            if (message.role === 'user' && (message.provenance ?? 'human') === 'human') {
                currentUserIndex = index;
            }
        });
    }
    const presentMessages = (messages: readonly Message[]): Message[] => messages.map(message => {
        if (!options.includeToolCallIds || message.role !== 'tool_result') {
            return message;
        }
    ...
    } | { kind: 'section'; section: PromptSection; };
    type Candidate = Item & {
        id: string;
        score: number;
        protected: boolean;
        ...
        references?: ContextDecision['references'];
        order: number
    };
    ...
    messageGroups.forEach((group, index) => {
        ...
        if ((!Number.isFinite(score) && score !== Infinity)) {
            throw new RangeError('Message group score must be finite or positive Infinity');
        }
        ...
        candidates.push({
            kind: 'messages',
            group,
            originalTokens: groupTokens(group.messages),
            id: `messages:${group.firstIndex}`,
            score,
    ...
    const render = (protectedOnly = false) => {
        ...
        for (const section of rendered.included) {
            ...
            systemSections.push({
                id: section.id,
                stability: section.stability ?? 'retained',
                start,
                end
            });
        }
        ...
        return {
            ...
            systemSections,
            messages,
            includedSections: rendered.included,
            usage
        };
    };
    if (options.maxToolResultCharacters !== undefined) {
        integer(options.maxToolResultCharacters, 'maxToolResultCharacters', 1);
        if (!options.referenceToolResult) {
            throw new TypeError('Tool output presentation requires referenceToolResult');
        }
        for (const item of candidates) {
            ...
            for (const [offset, message] of item.group.messages.entries()) {
                input.signal?.throwIfAborted();
                if (message.role !== 'tool_result' || message.contentBlocks?.length || message.content.length <= options.maxToolResultCharacters) {
                    continue;
                }
                ...
                if (typeof reference !== 'string' || !reference.trim()) {
                    throw new ContextCompressionError('Tool result reference must be nonempty text or null');
                }
                ...
                if (render().usage.totalTokens >= before) {
                    item.group.messages[offset] = message;
                    continue;
                }
    ...
    const referenceResults = (item: Candidate & { kind: 'messages' }, protectedResult = false, fits = () => result.usage.totalTokens <= budget) => {
        if (options.referenceToolResult) {
            for (const [offset, message] of item.group.messages.entries()) {
                ...
                if (message.role !== 'tool_result' || (!protectedResult && message.isError) || message.contentBlocks?.length
                ...
                if (content === null || content.length >= message.content.length) {
                    continue;
                }
                item.group.messages[offset] = { ...message, content };
                const trial = render();
                if (trial.usage.totalTokens >= result.usage.totalTokens) {
                    item.group.messages[offset] = message;
                    continue;
                }
                result = trial;
                ...
                item.compression ??= 'referenced';
                if (existing) {
                    existing.retainedCharacters = content.length;
                } else {
                    (item.references ??= []).push({
                        messageIndex,
                        reference,
    ...
    for (const item of candidates) {
        if (render(true).usage.totalTokens <= budget) {
            break;
        }
        if (item.protected && item.kind === 'messages') {
            referenceResults(item, true, () => render(true).usage.totalTokens <= budget);
        }
    }
    const minimum = render(true);
    if (minimum.usage.totalTokens > budget) {
        throw new ContextBudgetExceededError(budget, minimum.usage.totalTokens, 'protected');
    }
    const removable = candidates.filter(item => !item.protected).sort((left, right) => left.score - right.score
        || (left.kind === 'section' && right.kind === 'section' ? right.id.localeCompare(left.id) : left.order - right.order));
    for (const item of removable) {
        if (result.usage.totalTokens <= budget) {
            break;
        }
        input.signal?.throwIfAborted();
        if (item.kind === 'messages') {
            ...
            for (const [offset, message] of item.group.messages.entries()) {
                ...
                input.signal?.throwIfAborted();
                if (replacement !== null
                    && (!replacement || typeof replacement.content !== 'string'
                        || !isDeepStrictEqual(protectedShape(message), protectedShape(replacement)))) {
            ...
            if (trial.usage.totalTokens < result.usage.totalTokens) {
                ...
                item.action = 'compressed';
                item.reason = 'budget';
                item.compression = 'lossy';
            } else {
                item.group.messages = before;
            }
            ...
            if (compressed !== null) {
                if (typeof compressed !== 'string') {
                    throw new ContextCompressionError('Section compressor must return text or null');
                }
                const before = item.section;
                item.section = {
                    ...before,
                    text: () => compressed,
                    estimatedTokens: count(counter.countTokens(compressed))
                };
                const trial = render();
                if (trial.usage.totalTokens < result.usage.totalTokens) {
                    result = trial;
    ...
    if (result.usage.totalTokens > budget) {
        throw new ContextBudgetExceededError(budget, result.usage.totalTokens);
    }
    input.signal?.throwIfAborted();
    for (const item of candidates.filter(item => item.action === 'dropped')) {
        if (item.kind === 'messages') {
            for (const message of item.group.messages) {
                options.onDrop?.(structuredClone(message));
            }
        } else {
            options.onDropSection?.(snapshotSection(item.section));
        }
    }
    return {
        ...result,
        ...
        decisions: candidates.map(item => ({
            ...
            ...(item.kind === 'messages' ? {
                messageRange: { start: item.group.firstIndex, end: item.group.firstIndex + item.group.messages.length },
