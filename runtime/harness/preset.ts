...
export function defaultAgentExtensions(options: DefaultAgentOptions): Extension[] {
    ...
    return [
        ...
        { id: 'tools.coding', configuration: JSON.stringify({ workspace: options.workspace, outputDirectory: options.database ? `${options.database}.outputs` : null }), version: '5.0.0', apiVersion: 1, roles: { tools: () => codingToolRuntime(options.workspace, { outputDirectory: options.database ? `${options.database}.outputs` : undefined }) } },
