...
export async function defaultAgentExtensions(options: DefaultAgentOptions): Promise<Extension[]> {
    ...
    return [
        { id: 'sessions.local', version: '4.0.0', apiVersion: 1, roles: { store: () => options.database ? createSqliteSessionStore(options.database) : new MemorySessionStore() } },
        ...
        { id: 'context.budgeted', configuration: JSON.stringify({system, budget: options.tokenBudget ?? 24000, includeToolCallIds: true}), version: '9.0.0', apiVersion: 1, roles: { context: () => ({ ...budgetedContext(system, options.tokenBudget ?? 24000, { includeToolCallIds: true, referenceToolResult: archivedToolResultReference }), lifecycle: checkpointContextLifecycle({ maxTokens: 800, triggerRatio: 0.8 }) }) } },
