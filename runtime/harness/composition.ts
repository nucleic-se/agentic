...
export function compositionFingerprint<Roles extends object, Client extends HarnessClient>(extensions: readonly HarnessExtension<Roles, Client>[]): string {
    return createHash('sha256')
        .update(JSON.stringify(extensions
            .filter(e => Object.keys(e.roles ?? {}).length)
            .map(e => ({
                id: e.id,
                version: e.version,
                roles: Object.keys(e.roles ?? {}).sort(),
...
export async function composeDriver<Roles extends object, Client extends HarnessClient>(
    ...
    const roleNames = [...driver.roles];
    if (new Set(roleNames).size !== roleNames.length) throw new Error('Duplicate driver role');
    const extensions = options.extensions.map(extension => ({
        ...extension,
        requires: [...extension.requires ?? []],
        roles: { ...extension.roles } as NonNullable<HarnessExtension<Roles, Client>['roles']>,
    }));
    ...
    for (const extension of extensions) {
        ...
        for (const role of Object.keys(extension.roles)) {
            if (!roleNames.includes(role as keyof Roles & string)) throw new Error(`Unknown role: ${role}`);
    ...
    const missing = roleNames.filter(role => !owners.has(role));
    ...
    try {
        ...
        for (const extension of orderedExtensions) {
            for (const key of Object.keys(extension.roles) as (keyof Roles)[]) {
        ...
        client = await driver.start(roles, orderedExtensions);
        ...
        try {
            ...
            throw new AggregateError([error, cleanupError], 'Harness composition failed');
        }
        throw error;
    }
}
