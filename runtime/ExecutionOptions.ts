...
export function executionSignal(options: ProviderCallOptions = {}): { signal: AbortSignal; dispose(): void } {
    if (options.deadline !== undefined && !Number.isFinite(options.deadline)) throw new RangeError('Deadline must be a finite timestamp');
    const controller = new AbortController();
    const signal = options.signal ? options.deadline === undefined ? options.signal : AbortSignal.any([options.signal, controller.signal]) : controller.signal;
    ...
    if (options.deadline !== undefined) schedule();
    return { signal, dispose() { if (timer) clearTimeout(timer); } };
}
