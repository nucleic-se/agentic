...
export async function executeToolBatchDetailed(
    ...
    if (!Number.isSafeInteger(maxCalls) || maxCalls < 1) throw new RangeError('maxToolCallsPerTurn must be a positive safe integer');
    if (calls.length > maxCalls) throw new LLMProtocolError(`Provider returned ${calls.length} tool calls; maximum is ${maxCalls}`);
    const ids = new Set<string>();
    for (const call of calls) {
        if (ids.has(call.id)) throw new LLMProtocolError(`Provider returned duplicate tool call id '${call.id}'`);
        ids.add(call.id);
    }
    const signal = config.signal;
    const emit = config.emit ?? (() => {});
    const turnId = config.turnId ?? randomUUID();
    const prepared: PreparedCall[] = structuredClone(calls).map(call => ({ plan: {
        callId: call.id,
        name: call.name,
        input: call.args,
    ...
    if (!rawBatchRejected) {
        ...
        for (const item of prepared) {
            ...
            if (config.beforeToolCall) {
                ...
                try {
                    ...
                    item.blocked = { callId: item.plan.callId, plan: item.plan,
                        status: 'policy_denied', dispatched: false,
                        error: `beforeToolCall failed closed: ${errorMessage(error)}` };
                    continue;
                }
                if (hook.action === 'skip') {
                    item.blocked = { callId: item.plan.callId, plan: item.plan,
                        status: 'skipped', dispatched: false, error: hook.reason ?? 'Skipped by beforeToolCall hook' };
                    continue;
                }
            ...
            if (config.policy) {
                ...
                if (decision.kind === 'deny') {
                    item.blocked = { callId: item.plan.callId, plan: item.plan,
                        status: 'policy_denied', dispatched: false, error: decision.reason };
                    continue;
                }
                if (decision.kind === 'confirm') {
                    ...
                    if (!confirmed) {
                        item.blocked = { callId: item.plan.callId, plan: item.plan,
                            status: 'policy_denied', dispatched: false, error: confirmationError
        ...
        for (const item of prepared) {
            if (item.blocked || item.validation?.ok === false) continue;
            item.validation = safeValidate(config.tools, item.plan.name, item.plan.input as Record<string, unknown>);
            if (item.validation.ok && !isDeepStrictEqual(item.validation.args, item.authorizedInput)) {
                item.validation = { ok: false, result: { ok: false, content: 'Validation changed authorized arguments', errorKind: 'validation' } };
            }
            if (item.validation.ok) item.plan.input = item.validation.args;
        }
    }
    ...
    if (batchRejected) {
        ...
        for (const item of prepared) {
            ...
            await emit({ type: 'tool_start', turnId, callId: item.plan.callId,
                name: item.plan.name, input: structuredClone(item.plan.input) });
            if (signal?.aborted) { interruption = 'abort'; break; }
            ...
            if (config.afterToolCall) {
                try {
                    result = await config.afterToolCall({ ...contextForCall, result: structuredClone(rawResult), latencyMs }) ?? rawResult;
                } catch (error) {
                    result = rawResult;
            ...
            const execution: ToolExecution = {
                callId: item.plan.callId, plan: item.plan, status, dispatched: true, result, latencyMs,
            ...
            if (hookFailure) { controlFailure = hookFailure; break; }
            if (signal?.aborted) { interruption = 'abort'; break; }
        ...
        if (interruption || controlFailure) {
            for (const item of prepared.slice(executions.length)) {
                const execution: ToolExecution = {
                    ...
                    status: interruption === 'abort' ? 'cancelled' : 'skipped', dispatched: false,
    ...
    return { plans: prepared.map(item => item.plan), executions, interruption, steeringMessages,
