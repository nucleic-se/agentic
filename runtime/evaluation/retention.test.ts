...
it('repeats the same paired workload and distinguishes recovery from unavailable evidence', async () => {
    ...
    expect(retained.preparedEstimatedTokens).toBe(roomy.preparedEstimatedTokens);
    expect(retained.answerAvailable).toBe(true);
    expect(retained.retrievals).toBe(0);
    expect(first.cases.find(test => test.id === 'recoverable-2400')?.retrievals).toBeGreaterThan(0);
