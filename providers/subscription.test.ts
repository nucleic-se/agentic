...
function response(): Response {
    ...
    const events = [
        ...
        { type: 'response.completed', response: { id: 'resp_test', status: 'completed', output: [item], usage: {
            ...
            output_tokens_details: { reasoning_tokens: 3 }, total_tokens: 47,
        } } },
    ];
...
it('rejects malformed continuation before credentials or dispatch', async () => {
    let credentials = 0;
    const provider = new SubscriptionProvider({ model: 'fixture', credentials: async () => { credentials++; return token; }, fetch: async () => response() });
